---
name: frontend-skill
description: Use only when the user explicitly names `frontend-skill` or `$frontend-skill`. When explicitly invoked, guide visually strong landing pages, websites, apps, prototypes, demos, and game UIs with restrained composition, image-led hierarchy, cohesive content structure, and tasteful motion while avoiding generic cards, weak branding, and UI clutter.
---

# Frontend Skill

Use this skill only when the user explicitly asks for `frontend-skill` or `$frontend-skill`.
